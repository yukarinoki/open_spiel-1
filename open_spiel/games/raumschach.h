...
#ifndef OPEN_SPIEL_GAMES_RAUMSCHACH_H_
#define OPEN_SPIEL_GAMES_RAUMSCHACH_H_
...
class RaumschachState : public State {
 public:
  RaumschachState(std::shared_ptr<const Game> game);

  RaumschachState(const RaumschachState&) = default;
  RaumschachState& operator=(const RaumschachState&) = default;
...
// Game object.
class RaumschachGame : public Game {
 public:
  explicit RaumschachGame(const GameParameters& params);
  int NumDistinctActions() const override { return kNumCells; }
  std::unique_ptr<State> NewInitialState() const override {
    return std::unique_ptr<State>(new RaumschachState(shared_from_this()));
  }
...
#endif  // OPEN_SPIEL_GAMES_RAUMSCHACH_H_
